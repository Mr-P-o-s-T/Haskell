...
insertEdge i j w = Map.insertWith Map.union i (Map.singleton j w)
...
randomGraph g max_vertex max_weight edges = (mat, vs)
  where
    (g1, g2) = split g
    (g3, g4) = split g2
    is = take edges (randomRs (0, max_vertex) g1) ++ [0 .. max_vertex]
    js = take edges (randomRs (0, max_vertex) g3) ++ [0 .. max_vertex]
    ws = take edges (randomRs (1, max_weight) g4) ++ replicate (max_vertex + 1) 0
    mat = foldr ins Map.empty (zip3 is js ws)
      where
        ins (i, j, w) = insertEdge i j w
...
printRow :: Int -> [Int] -> IO ()
printRow _ [] = putStrLn ""
printRow 0 row = putStrLn "\t..."
printRow n (x:row) = do
  putStr ("\t" ++ show x)
  printRow (n - 1) row

printMatrixXY :: Int -> Int -> [[Int]] -> IO ()
printMatrixXY n _ [] = putStr ""
printMatrixXY n 0 row = do
  putStr $ unwords $ replicate (n `div` 2) "\t"
  putStrLn "."
  putStr $ unwords $ replicate (n `div` 2) "\t"
  putStrLn "."
  putStr $ unwords $ replicate (n `div` 2) "\t"
  putStrLn "."
printMatrixXY n m (row:rows) = do
  printRow n row
  printMatrixXY n (m - 1) rows

printMatrix :: Int -> [[Int]] -> IO ()
printMatrix n = printMatrixXY n n
